...
# appears in each text. This will be an approximation because it will also count words such as ‘then’ and ‘tehre’.
# Try counting ‘the ‘, with a space in the string, and see how much lower your count is.

def count_the(filename):
    """Counts the number of times "the" appears in .txt files"""
    try:
        ...
        print(f"In {filename} 'the' appears {counter} times.")
        print("hoho")


filenames = ['CommonWords1.txt', 'CommonWords2.txt']
for filename in filenames:
    count_the(filename)
    # print(count_the(filename)) => vraca None nakon što izvrši funkciju